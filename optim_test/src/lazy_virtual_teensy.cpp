// lazy_virtual_teensy.cpp: A made-up teensy model which does "lazy" work, that is,
// it does not do the specified worrk in time and sometimes loses his tasks.
...
void teensy_callback( const teensy_msgs::CommandMsg::ConstPtr & receivedMsg )
{
    ...
    // Reflecting the CommandMsg. The code here might be a little awkward but it is really what should be done for MC code.
    for( Uint8 i=0; i<pp.kondoServoNum; i++ )
    {
    ...
    // But this teensy is lazy. He sleeps a bit more.
    ( receivedMsg->expectedSceneDuration * (rand()%5/10.0) );
